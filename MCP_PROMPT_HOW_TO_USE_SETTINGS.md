...
10. **Create a new secret with a generated random value** — call `create_random_secret` with `(product_id, secret_id, length, level, description, visible_for_mcp)` when the value is a credential nobody has to choose by hand: a password, an API token, a salt. The service generates the value itself from `[0-9a-zA-Z]` and never returns it — you only say how long it must be (32-64 for real credentials) and which `level` it gets. The call is idempotent and never destructive: when the secret already exists it is left exactly as it is (no new value generated, nothing overwritten) and the response comes back with `created: false` and the metadata of the existing secret, so it is safe to call on every provisioning run. Keep `visible_for_mcp: false` unless the generated value must be copied into another system by the AI.

11. **Move a secret between scopes** — call `move_secret` with `(secret_id, from_product_id, to_product_id)` to relocate a secret without recreating it. Typical uses: promote a product-scoped secret to `"Shared"` so other products can reuse it, or pull a shared secret down into a single product. The id and every field (value, remote variant, level, description, visibility, timestamps) are preserved; only the scope changes. The call fails when the source secret is missing, when source and target are the same scope, or when the target already holds a secret with that id (it NEVER overwrites). By default it also refuses the move if it would break a `${...}` reference — either a dependency the secret needs (e.g. promoting to `Shared` a secret that references a product-only secret) or another secret/template that consumes it (e.g. pulling a shared secret into one product breaks consumers in other products); the error lists exactly what would break. Pass `force: true` to move anyway, knowingly leaving those references unresolved.

12. **Record product context** — call `upsert_product` with `(product_id, description, prompt)` to create or update the explicit description and prompt for a product. The prompt should explain what the product is and how its secrets/templates are organised — future agents will read it via `get_product_prompt`.
